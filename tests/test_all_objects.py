...
from baba.all_objects import (
    ALL_OBJECTS, ALL_TEXT_OBJECTS,
    GameObject, TextGameObject,
    # Test a few specific objects
    BabaObject, KekeObject, WallObject,
    BabaTextObject, IsTextObject, YouTextObject
)
...
class TestAllObjects:
    ...
    def test_object_ids_match(self):
        ...
        for name, obj_class in ALL_OBJECTS.items():
            ...
            if hasattr(obj, 'type_id'):
                ...
                if expected_name not in ['empty', 'cursor'] and not expected_name.startswith('default'):
                    assert obj.name == expected_name, f"Name mismatch: {obj.name} != {expected_name}"
    ...
    def test_unique_type_ids(self):
        ...
        for name, obj_class in ALL_OBJECTS.items():
            ...
            if obj.type_id in seen_ids:
                pytest.fail(f"Duplicate type_id {obj.type_id} in {name} and {seen_ids[obj.type_id]}")
    ...
    def test_property_text_objects(self):
        ...
        property_names = [
            "you", "win", "stop", "push", "sink", "defeat",
            "hot", "melt", "move", "float", "shift", "tele",
            "pull", "open", "shut", "weak"
        ]
