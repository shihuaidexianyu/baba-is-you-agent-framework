...
from .rendering import get_icon_for_object
...
class Grid:
    ...
    def find_objects(self, obj_type: Optional[Object] = None, name: Optional[str] = None) -> List[Tuple[Object, int, int]]:
        """
        ...
        for y in range(self.height):
            for x in range(self.width):
                for obj in self.grid[y][x]:
                    if obj_type and obj.type_id == obj_type.type_id:
                        results.append((obj, x, y))
                    elif name and obj.name.upper() == name.upper():
                        results.append((obj, x, y))
                    elif not obj_type and not name:
                        results.append((obj, x, y))
        return results
    ...
    def render(self, cell_size: int = 24) -> np.ndarray:
        ...
        img = np.full((self.height * cell_size, self.width * cell_size, 3), 
            ...
            for x in range(self.width):
                ...
                if objects:
                    ...
                    if len(objects) > 1:
                        ...
                        cv2.circle(img, (x_start + cell_size - 4, y_start + 4), 
                                  2, (255, 255, 255), -1)
