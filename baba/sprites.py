...
def create_object_sprite(
    obj_name: str, color: tuple[int, int, int], size: tuple[int, int] = (24, 24)
) -> np.ndarray:
    """
...
def create_text_sprite(
    ...
    if pattern:
        # Create from pattern
        text_sprite = create_sprite_from_pattern(
            pattern, color, (0, 0, 0), (size[0] - 4, size[1] - 4)
        )
        # Center it
...
def add_3d_effect(sprite: np.ndarray, base_color: tuple[int, int, int]) -> np.ndarray:
    """
    ...
    for y in range(h):
        for x in range(w):
            if np.array_equal(sprite[y, x], base_color):
                ...
                if (
                    y > 0
                    and not np.array_equal(sprite[y - 1, x], base_color)
                    or x > 0
                    and not np.array_equal(sprite[y, x - 1], base_color)
                ):
...
def create_animated_sprite(
    obj_name: str, color: tuple[int, int, int], frame: int = 0, size: tuple[int, int] = (24, 24)
) -> np.ndarray:
    """
